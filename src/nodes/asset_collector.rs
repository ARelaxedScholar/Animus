...
impl AssetCollectorLogic {
    ...
    async fn download_with_retry(&self, url: &str, max_retries: u32) -> Result<Vec<u8>, String> {
        ...
        for attempt in 0..max_retries {
            info!(
                "AssetCollector: Download attempt {}/{} for {}",
                attempt + 1,
                max_retries,
                url
            );
            match self.http_client.get(url).send().await {
                Ok(response) => {
                    if response.status().is_success() {
                        match response.bytes().await {
                            Ok(bytes) => {
                                info!(
                                    "AssetCollector: Download successful on attempt {}",
                                    attempt + 1
                                );
                            ...
                            Err(e) => {
                                ...
                                warn!(
                                    "AssetCollector: Failed to read bytes on attempt {}: {}",
                                    attempt + 1,
                                    e
                                );
                            }
                        ...
                        warn!(
                            "AssetCollector: HTTP error on attempt {}: {}",
                            attempt + 1,
                            response.status()
                        );
                    }
                ...
                Err(e) => {
                    ...
                    warn!(
                        "AssetCollector: Request failed on attempt {}: {}",
                        attempt + 1,
                        e
                    );
                }
            ...
            if attempt < max_retries - 1 {
                ...
                info!(
                    "AssetCollector: Retrying after {} seconds...",
                    delay.as_secs()
                );
        ...
        error!(
            "AssetCollector: All {} download attempts failed for {}",
            max_retries, url
        );
    ...
    fn validate_video_bytes(&self, bytes: &[u8], min_size_kb: u64) -> Result<String, String> {
        ...
        info!(
            "AssetCollector: Video validation passed - size: {}KB, hash: {}",
            size_kb, hash
        );
        Ok(hash)
    }
    ...
    fn clean_search_query(suggestion: &str) -> String {
        ...
        let phrases_to_remove = ["(if available)", "(optional)", "or similar"];
    ...
    async fn enrich_visual_intent(
        &self,
        section: &crate::nodes::ScriptSection,
    ) -> Result<VisualIntent, String> {
  ...
  "sfx_triggers": [
    { "sfx_type": "texture|punctuation", "sound": "...", "relative_time": 0.5, "volume": 0.3 }
        ...
        let response = self
            ...
            .temperature(0.7)
            .await
            .map_err(|e| format!("Gemini enrichment failed: {}", e))?;
        ...
        serde_json::from_str(cleaned).map_err(|e| {
            format!(
                "Failed to parse visual intent: {}. Response: {}",
                e, response
            )
        })
    }
    ...
    async fn generate_leonardo_image(
        &self,
        prompt: &str,
        video_id: &str,
        section_idx: usize,
        ...
        let api_key = self
            ...
            .leonardo_api_key
            .as_ref()
            .ok_or("No Leonardo.ai API key")?;

        info!(
            "AssetCollector: Generating AI image with Leonardo.ai for section {}...",
            section_idx
        );
        ...
        let gen_res: serde_json::Value = response
            .json()
            .await
            .map_err(|e| format!("Failed to parse Leonardo response: {}", e))?;
        let generation_id = gen_res["sdGenerationJob"]["generationId"]
            .as_str()
        ...
        for _ in 0..20 {
            ...
            let poll_json: serde_json::Value = poll_res
                .json()
                .await
                .map_err(|e| format!("Failed to parse poll response: {}", e))?;
            let status = poll_json["generations_by_pk"]["status"]
                .as_str()
                .unwrap_or("PENDING");

            if status == "COMPLETE" {
                image_url = poll_json["generations_by_pk"]["generated_images"][0]["url"]
                    .as_str()
                    .map(|s| s.to_string());
                break;
            }
        ...
        let img_data = self
            ...
            .bytes()
            .await
            .map_err(|e| format!("Failed to read AI image bytes: {}", e))?;

        let key = format!("assets/{}/section_{}/ai_image.png", video_id, section_idx);
        self.s3_client
            .upload_bytes(&key, img_data.to_vec(), "image/png")
            .await
    ...
    async fn search_pexels_videos(
        ...
        let pexels_response: PexelsVideoResponse = serde_json::from_str(&body).map_err(|e| {
            // Log the first 500 chars of the response for debugging
            let preview = utils::safe_truncate(&body, 500);
            format!(
                "Failed to parse Pexels response: {}. Body preview: {}",
                e, preview
            )
        })?;
    ...
    async fn download_and_upload_video(
        ...
        let video_files = video
            .video_files
            .as_ref()
            .ok_or("No video files in response")?;
        ...
        let video_bytes = self
            .download_with_retry(link, self.config.max_retries)
            .await?;
        ...
        info!(
            "AssetCollector: Video downloaded and validated - size: {:.2} MB",
        ...
        self.s3_client
            .upload_bytes(&key, data, "video/mp4")
            .await
...
impl AsyncNodeLogic for AssetCollectorLogic {
    ...
    async fn exec(&self, input: NodeValue) -> NodeValue {
        ...
        if let Some(existing) = input
            .get("existing_manifest")
            .and_then(|v| serde_json::from_value::<AssetManifest>(v.clone()).ok())
        {
            if !existing.section_assets.is_empty() {
                info!(
                    "AssetCollector: Existing manifest found for video {}, skipping collection",
                    script.video_id
                );
        ...
        info!(
            "AssetCollector: Gathering assets for video {}",
            script.video_id
        );
        ...
        for (section_idx, section) in all_sections.iter().enumerate() {
            ...
            let intent = match self.enrich_visual_intent(section).await {
                ...
                Err(e) => {
                    warn!(
                        "Failed to enrich visual intent for section {}: {}",
                        section_idx, e
                    );
                    VisualIntent {
            ...
            if (video_clips.is_empty() || video_clips.len() < 2) && self.config.fallback_to_images {
                match self
                    .generate_leonardo_image(&intent.ai_image_prompt, &video_id, section_idx)
                    .await
                {
        ...
        if let Some(error) = exec_res.get("error").and_then(|v| v.as_str()) {
            ...
            if let Some(vid) = shared.get(state_keys::VIDEO_ID).and_then(|v| v.as_str()) {
                if let Ok(video_id) = uuid::Uuid::parse_str(vid) {
                    let _ =
                        db::mark_video_failed(&self.db_pool, video_id, "asset_collector", error)
                            .await;
                }
            }
...
mod tests {
    ...
    fn test_clean_search_query_archive_footage() {
        assert_eq!(
            AssetCollectorLogic::clean_search_query(
                "Archive footage of Viktor Frankl (if available)"
            ),
            "Viktor Frankl"
        );
    }
    ...
    fn test_clean_search_query_wide_shot() {
        assert_eq!(
            AssetCollectorLogic::clean_search_query(
                "Wide shot of a desolate but beautiful desert landscape"
            ),
    ...
    fn test_clean_search_query_macro_shots() {
        ...
        let result = AssetCollectorLogic::clean_search_query(
            "Macro shots of nature: an eye of a hawk, a rushing river",
        );
    ...
    fn test_pexels_response_parsing_success() {
        ...
        assert_eq!(
            response.videos[0].video_files.as_ref().unwrap()[0].width,
            Some(1920)
        );
    }
    ...
    fn test_pexels_response_parsing_extra_fields() {
        ...
        assert!(response.videos[0].video_files.as_ref().unwrap()[0]
            .link
            .as_ref()
            .unwrap()
            .contains("vimeo"));
